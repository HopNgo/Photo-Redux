...
const photo = createSlice({
    ...
    reducers: {
        ...
        removePhoto: (state, action) => {
            const photoRemoveID = action.payload;
            return state.filter(photo => photo.id !== photoRemoveID)
        },
        updatePhoto: (state, action) => {
            const photoUpdate = action.payload;
            const indexPhotoUpdate = state.findIndex(photo => photo.id === photoUpdate.id);
            if (indexPhotoUpdate >= 0)
                state[indexPhotoUpdate] = photoUpdate;
        }
    }
})
